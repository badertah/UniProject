...
5. **Arcade-Style SAD Games — Per-Question Mini-Stages** (all in `client/src/components/sad-games.tsx`, dispatched via `SADGameRunner`). Each seeded question is now its own playable mini-stage with its own card / score / XP / checkpoint. URL routing: `/game/:id?stage=N`. Difficulty ramps within each game via `difficulty = stageIndex / (totalStages - 1)` (0..1) → EASY / NORMAL / HARD / EXPERT pill (`difficultyLabel(d)`). Each game scales spawn rate, speed, hit window, hearts, and timer with `difficulty`. Topic page shows 4-up stage chips per SAD level with shimmer-on-completed and primary-border-on-next-up. Backend stores one row per (`userId`, `levelId`, `stageIndex`) — see Per-Stage Progress below. Each game binds directly to seeded question data (`q.content`, `q.answer`, `q.options`) — no random hardcoded content. All games share a small infra layer:
   ...
   - **Spec Sort** (`req_sorter`, component `RequirementHighway`) — 2D vertical conveyor arcade. Large readable requirement cards drop from the top down a center channel; up to 4 cards on screen at once with slight per-card speed variation. The lowest un-decided card carries a "▶ NEXT" marker — that's the one your verdict applies to. Press ←/A or →/D (or tap left/right side) to fling it into the matching bin: Functional ⚙️ left, Non-Functional 📊 right. A yellow **SWEET ZONE** band just above the floor doubles the score for cards sorted there. Decided cards fly off into their bin with a particle burst. **Endless wave system**: every 8 successful sorts clears a wave (+50 bonus, banner, faster fall + tighter spawn gap). Combo: 3-in-a-row gives +5 per sort, every 5-combo heals +1 ❤ (capped at 5). Hearts: max 5; wrong sort OR card crossing the floor = -1 heart, combo reset, screen shake. Question pool cycles infinitely. Game ends when hearts hit 0; final score = sum of sort points + wave bonuses. All visuals are clean 2D (no shrinking perspective) so card text stays readable throughout.
   ...
   - **System Architect** (`system_architect`, lives in `client/src/components/system-architect.tsx`) — Tower-defense / sim. Sidebar shop sells **Firewall** ($60 / 220 r/s), **Load Balancer** ($90 / ∞), **Web Server** ($70 / 55 r/s), **Database** ($130 / 110 r/s). 8×5 grid workspace; Internet pre-placed at column 0. Tools: **Place** (click shop → click cell), **Wire** (click source → click target, animated arrow with traffic dots), **Sell** (50% refund), **Select**. Per-tick simulation: BFS from Internet propagates traffic; LB / Internet split equally to all alive children; capacity-bound nodes only forward `min(load, capacity)`. Overload: heat += `(load-cap) * 1.4 * dt`; cool: heat -= `14 * dt`. Heat = 100 → 💥 component crashes (greys out, dashed outgoing wires). DB earns `$0.18 * load * dt`. Traffic ramps `+4 r/s` every 8s with floating "+TRAFFIC" warning. Game-over only when at least one DB exists AND no living Internet→DB path remains. Score = `time*5 + reqs*0.4 + earnings*0.5`. Heat bars on every component; live load number badge; pause (Space) / cancel-tool (Esc); how-to overlay on first run. Difficulty knob: harder runs start with more traffic and less budget. Seeded as SAD level 8 (`hard`, +110 XP / +35 coins) by the idempotent `seedSystemArchitectLevel()` in `server/seed.ts` so existing DBs pick up the new game on next boot.
