...
11. **Farm Tycoon 2.5D** — immersive full-viewport farm game (`/farm`): parallax sky with animated clouds, sun, birds; green landscape with decorative trees, butterflies, grass tufts; 12 buildings on a centered 4×3 grid with dirt paths; animated farm life (chickens walking, cows grazing, windmill spinning, tractor driving, water drops) that appear when respective buildings are owned; floating sparkle/leaf particles; CSS keyframe animations for all ambient elements; game-style HUD overlay (income/min, buildings, farm bank, coins, harvest button); 30s passive income ticks; farm bank → Harvest → EduCoins (capped 500/harvest); offline income catch-up (max 20 ticks); building detail modal with buy/upgrade; state in localStorage `farm_v2_state`; SVG buildings use React `useId()` for unique gradient/filter IDs; `/api/farm/harvest` endpoint
