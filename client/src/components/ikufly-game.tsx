...
import { X, Zap, Coins, Trophy, Play, RotateCcw } from "lucide-react";
...
function drawBird(ctx: CanvasRenderingContext2D, y: number, vel: number) {
  const tilt = Math.max(-30, Math.min(60, vel * 4));
  ctx.save();
  ctx.translate(BIRD_X, y);
  ctx.rotate((tilt * Math.PI) / 180);

  // Body — graduation cap book bird
...
function drawPipe(ctx: CanvasRenderingContext2D, pipe: Pipe) {
  ...
  ctx.fillRect(pipe.x + 3, 0, PIPE_W, pipe.topH);
  ctx.fillRect(pipe.x + 3, bottomY, PIPE_W, totalH - bottomY);

  // Pipe body — book/pencil theme
  ...
  ctx.fillRect(pipe.x, bottomY, PIPE_W, totalH - bottomY);

  // Pipe caps
  ctx.fillStyle = "#2563eb";
  ctx.fillRect(pipe.x - 4, pipe.topH - 14, PIPE_W + 8, 14);
  ctx.fillRect(pipe.x - 4, bottomY, PIPE_W + 8, 14);

  // Highlight stripe
  ctx.fillStyle = "rgba(255,255,255,0.15)";
  ctx.fillRect(pipe.x + 6, 0, 6, pipe.topH);
  ctx.fillRect(pipe.x + 6, bottomY, 6, totalH - bottomY);

  // Small book icons on pipes
  ctx.fillStyle = "#60a5fa";
  ctx.font = "bold 11px sans-serif";
  ctx.textAlign = "center";
  if (pipe.topH > 50) ctx.fillText("📚", pipe.x + PIPE_W / 2, pipe.topH / 2);
}
...
function drawBackground(ctx: CanvasRenderingContext2D, offset: number) {
  ...
  const stars = [[20, 30], [80, 60], [160, 20], [240, 80], [300, 40], [340, 90], [50, 120], [200, 50]];
  stars.forEach(([sx, sy]) => {
    ctx.beginPath();
    ctx.arc((sx + offset * 0.2) % W, sy, 1.2, 0, Math.PI * 2);
    ctx.fill();
  });
  ...
  // Ground pattern
  for (let gx = (-offset * 1.5) % 60; gx < W; gx += 60) {
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      if (phase === "playing") {
        ...
        // Floating animation
        s.birdY = H / 2 + Math.sin(Date.now() / 400) * 10;
      }
    ...
    <motion.div
      ...
      <motion.div
        ...
        <div className="relative" style={{ width: W, height: H }}>
              ...
              <motion.div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none"
                ...
                <div className="text-center px-8">
                  ...
                  <div className="bg-white/10 rounded-lg px-4 py-2 text-xs text-slate-300 border border-white/10">
                    Avoid the lecture slides and fly through!<br />
          ...
          <AnimatePresence>
            {displayPhase === "dead" && (
              <motion.div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 pointer-events-none"
                initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <div className="text-center bg-[#0f0c29] border border-purple-500/30 rounded-2xl p-6 mx-6 pointer-events-auto">
